...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === 'GET' && req.url === '/dogs') {
      ...
      const resBody=dogsPage.replace(/#{dogsList}/g,dogs.map(dog => `<li>${dog.name}</li>`).join(''));
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        // Your code here
        const dogDetailsPage=fs.readFileSync("./views/dog-details.html", 'utf-8');
        const resBody=dogDetailsPage.replace(/#{name}/g,dog.name).replace(/#{age}/g,dog.age);
